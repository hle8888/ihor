// Generated by HLC 4.3.7 (HL v4)
#define HLC_BOOT
#include <hlc.h>
#include <haxe/format/JsonParser.h>
vdynamic* haxe_format_JsonParser_parseRec(haxe__format__JsonParser);
void haxe_format_JsonParser_invalidChar(haxe__format__JsonParser);
#include <hl/types/ArrayDyn.h>
#include <hl/types/ArrayObj.h>
String haxe_format_JsonParser_parseString(haxe__format__JsonParser);
void haxe_format_JsonParser_invalidNumber(haxe__format__JsonParser,int);
extern hl_type t$_i32;
String String_substr(String,int,vdynamic*);
double Std_parseFloat(String);
extern hl_type t$_f64;
extern hl_type t$_dyn;
#include <hl/natives.h>
hl__types__ArrayObj hl_types_ArrayObj_alloc(varray*);
#include <hl/types/ArrayBase.h>
hl__types__ArrayDyn hl_types_ArrayDyn_alloc(hl__types__ArrayBase,bool*);
int hl_types_ArrayDyn_push(hl__types__ArrayDyn,vdynamic*);
void Reflect_setField(vdynamic*,String,vdynamic*);
#include <_std/StringBuf.h>
extern hl_type t$StringBuf;
void StringBuf_new(StringBuf);
void StringBuf_addSub(StringBuf,String,int,vdynamic*);
void StringBuf_addChar(StringBuf,int);
extern String s$Invalid_escape_sequence_;
String String_fromCharCode(int);
String String___add__(String,String);
extern String s$_at_position_;
String String___alloc__(vbyte*,int);
extern String s$0x;
vdynamic* Std_parseInt(String);
extern String s$Unclosed_string;
String StringBuf_toString(StringBuf);
extern String s$Invalid_char_;
extern String s$Invalid_number_at_position_;
extern String s$e265492;

void haxe_format_JsonParser_new(haxe__format__JsonParser r0,String r1) {
	int r2;
	r0->str = r1;
	r2 = 0;
	r0->pos = r2;
	return;
}

vdynamic* haxe_format_JsonParser_doParse(haxe__format__JsonParser r0) {
	String r7;
	bool r3;
	vdynamic *r1;
	vbyte *r6;
	int r4, r5, r8, r9;
	r1 = haxe_format_JsonParser_parseRec(r0);
	label$36f55b1_2_1:
	r3 = true;
	if( !r3 ) goto label$36f55b1_2_21;
	r4 = r0->pos;
	r5 = r4;
	++r4;
	r0->pos = r4;
	r7 = r0->str;
	if( r7 == NULL ) hl_null_access();
	r6 = r7->bytes;
	r9 = 1;
	r8 = r5 << r9;
	r8 = *(unsigned short*)(r6 + r8);
	r9 = 0;
	if( r8 != r9 ) goto label$36f55b1_2_17;
	goto label$36f55b1_2_21;
	label$36f55b1_2_17:
	switch(r8) {
		default:
		case 0:
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 8:
		case 11:
		case 12:
		case 14:
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
		case 20:
		case 21:
		case 22:
		case 23:
		case 24:
		case 25:
		case 26:
		case 27:
		case 28:
		case 29:
		case 30:
		case 31:
			haxe_format_JsonParser_invalidChar(r0);
			goto label$36f55b1_2_20;
		case 9:
			break;
		case 10:
			break;
		case 13:
			break;
		case 32:
			break;
	}
	label$36f55b1_2_20:
	goto label$36f55b1_2_1;
	label$36f55b1_2_21:
	return r1;
}

vdynamic* haxe_format_JsonParser_parseRec(haxe__format__JsonParser r0) {
	vdynobj *r33;
	bool *r29;
	String r6, r34;
	hl__types__ArrayObj r28;
	hl_type *r27;
	bool r1, r13, r14, r15, r16, r17, r18, r21;
	hl__types__ArrayDyn r25;
	double r22, r24;
	vdynamic *r9, *r23, *r30, *r31, *r32, *r35;
	varray *r26;
	vbyte *r5, *r7, *r19;
	int r2, r3, r8, r10, r11, r12, r20;
	label$36f55b1_3_0:
	r1 = true;
	if( !r1 ) goto label$36f55b1_3_407;
	r2 = r0->pos;
	r3 = r2;
	++r2;
	r0->pos = r2;
	r6 = r0->str;
	if( r6 == NULL ) hl_null_access();
	r5 = r6->bytes;
	r8 = 1;
	r3 = r3 << r8;
	r3 = *(unsigned short*)(r5 + r3);
	switch(r3) {
		default:
		case 0:
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 8:
		case 11:
		case 12:
		case 14:
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
		case 20:
		case 21:
		case 22:
		case 23:
		case 24:
		case 25:
		case 26:
		case 27:
		case 28:
		case 29:
		case 30:
		case 31:
		case 33:
		case 35:
		case 36:
		case 37:
		case 38:
		case 39:
		case 40:
		case 41:
		case 42:
		case 43:
		case 44:
		case 46:
		case 47:
		case 58:
		case 59:
		case 60:
		case 61:
		case 62:
		case 63:
		case 64:
		case 65:
		case 66:
		case 67:
		case 68:
		case 69:
		case 70:
		case 71:
		case 72:
		case 73:
		case 74:
		case 75:
		case 76:
		case 77:
		case 78:
		case 79:
		case 80:
		case 81:
		case 82:
		case 83:
		case 84:
		case 85:
		case 86:
		case 87:
		case 88:
		case 89:
		case 90:
		case 92:
		case 93:
		case 94:
		case 95:
		case 96:
		case 97:
		case 98:
		case 99:
		case 100:
		case 101:
		case 103:
		case 104:
		case 105:
		case 106:
		case 107:
		case 108:
		case 109:
		case 111:
		case 112:
		case 113:
		case 114:
		case 115:
		case 117:
		case 118:
		case 119:
		case 120:
		case 121:
		case 122:
			haxe_format_JsonParser_invalidChar(r0);
			goto label$36f55b1_3_406;
		case 9:
		case 10:
		case 13:
		case 32:
			goto label$36f55b1_3_406;
		case 34:
			r6 = haxe_format_JsonParser_parseString(r0);
			return ((vdynamic*)r6);
		case 45:
		case 48:
		case 49:
		case 50:
		case 51:
		case 52:
		case 53:
		case 54:
		case 55:
		case 56:
		case 57:
			r10 = r0->pos;
			r11 = 1;
			r10 = r10 - r11;
			r12 = 45;
			if( r3 == r12 ) goto label$36f55b1_3_26;
			r1 = false;
			goto label$36f55b1_3_27;
			label$36f55b1_3_26:
			r1 = true;
			label$36f55b1_3_27:
			r13 = !r1;
			r12 = 48;
			if( r3 == r12 ) goto label$36f55b1_3_32;
			r14 = false;
			goto label$36f55b1_3_33;
			label$36f55b1_3_32:
			r14 = true;
			label$36f55b1_3_33:
			r15 = false;
			r16 = false;
			r17 = false;
			r18 = false;
			label$36f55b1_3_37:
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r7 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r7 + r12);
			switch(r12) {
				default:
				case 0:
				case 1:
				case 2:
				case 3:
				case 4:
				case 5:
				case 6:
				case 7:
				case 8:
				case 9:
				case 10:
				case 11:
				case 12:
				case 13:
				case 14:
				case 15:
				case 16:
				case 17:
				case 18:
				case 19:
				case 20:
				case 21:
				case 22:
				case 23:
				case 24:
				case 25:
				case 26:
				case 27:
				case 28:
				case 29:
				case 30:
				case 31:
				case 32:
				case 33:
				case 34:
				case 35:
				case 36:
				case 37:
				case 38:
				case 39:
				case 40:
				case 41:
				case 42:
				case 44:
				case 47:
				case 58:
				case 59:
				case 60:
				case 61:
				case 62:
				case 63:
				case 64:
				case 65:
				case 66:
				case 67:
				case 68:
				case 70:
				case 71:
				case 72:
				case 73:
				case 74:
				case 75:
				case 76:
				case 77:
				case 78:
				case 79:
				case 80:
				case 81:
				case 82:
				case 83:
				case 84:
				case 85:
				case 86:
				case 87:
				case 88:
				case 89:
				case 90:
				case 91:
				case 92:
				case 93:
				case 94:
				case 95:
				case 96:
				case 97:
				case 98:
				case 99:
				case 100:
					if( r13 ) goto label$36f55b1_3_51;
					haxe_format_JsonParser_invalidNumber(r0,r10);
					label$36f55b1_3_51:
					r12 = r0->pos;
					--r12;
					r0->pos = r12;
					r21 = true;
					r18 = r21;
					goto label$36f55b1_3_104;
				case 43:
				case 45:
					if( !r16 ) goto label$36f55b1_3_59;
					if( !r17 ) goto label$36f55b1_3_60;
					label$36f55b1_3_59:
					haxe_format_JsonParser_invalidNumber(r0,r10);
					label$36f55b1_3_60:
					r21 = false;
					r13 = r21;
					r21 = true;
					r17 = r21;
					goto label$36f55b1_3_104;
				case 46:
					if( r1 ) goto label$36f55b1_3_68;
					if( r15 ) goto label$36f55b1_3_68;
					if( !r16 ) goto label$36f55b1_3_69;
					label$36f55b1_3_68:
					haxe_format_JsonParser_invalidNumber(r0,r10);
					label$36f55b1_3_69:
					r21 = false;
					r13 = r21;
					r21 = true;
					r15 = r21;
					goto label$36f55b1_3_104;
				case 48:
					if( !r14 ) goto label$36f55b1_3_77;
					if( r15 ) goto label$36f55b1_3_77;
					haxe_format_JsonParser_invalidNumber(r0,r10);
					label$36f55b1_3_77:
					if( !r1 ) goto label$36f55b1_3_82;
					r21 = false;
					r1 = r21;
					r21 = true;
					r14 = r21;
					label$36f55b1_3_82:
					r21 = true;
					r13 = r21;
					goto label$36f55b1_3_104;
				case 49:
				case 50:
				case 51:
				case 52:
				case 53:
				case 54:
				case 55:
				case 56:
				case 57:
					if( !r14 ) goto label$36f55b1_3_88;
					if( r15 ) goto label$36f55b1_3_88;
					haxe_format_JsonParser_invalidNumber(r0,r10);
					label$36f55b1_3_88:
					if( !r1 ) goto label$36f55b1_3_91;
					r21 = false;
					r1 = r21;
					label$36f55b1_3_91:
					r21 = true;
					r13 = r21;
					r21 = false;
					r14 = r21;
					goto label$36f55b1_3_104;
				case 69:
				case 101:
					if( r1 ) goto label$36f55b1_3_99;
					if( r14 ) goto label$36f55b1_3_99;
					if( !r16 ) goto label$36f55b1_3_100;
					label$36f55b1_3_99:
					haxe_format_JsonParser_invalidNumber(r0,r10);
					label$36f55b1_3_100:
					r21 = false;
					r13 = r21;
					r21 = true;
					r16 = r21;
			}
			label$36f55b1_3_104:
			if( r18 ) goto label$36f55b1_3_106;
			goto label$36f55b1_3_37;
			label$36f55b1_3_106:
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r12 = r0->pos;
			r12 = r12 - r10;
			r23 = hl_alloc_dynamic(&t$_i32);
			r23->v.i = r12;
			r6 = String_substr(r6,r10,r23);
			r22 = Std_parseFloat(r6);
			if( !r15 ) goto label$36f55b1_3_116;
			r9 = hl_alloc_dynamic(&t$_f64);
			r9->v.d = r22;
			return r9;
			label$36f55b1_3_116:
			r11 = (int)r22;
			r24 = (double)r11;
			if( r24 != r22 ) goto label$36f55b1_3_121;
			r9 = hl_alloc_dynamic(&t$_i32);
			r9->v.i = r11;
			return r9;
			label$36f55b1_3_121:
			r9 = hl_alloc_dynamic(&t$_f64);
			r9->v.d = r22;
			return r9;
		case 91:
			r27 = &t$_dyn;
			r8 = 0;
			r26 = hl_alloc_array(r27,r8);
			r28 = hl_types_ArrayObj_alloc(r26);
			r1 = true;
			r29 = &r1;
			r25 = hl_types_ArrayDyn_alloc(((hl__types__ArrayBase)r28),r29);
			r30 = NULL;
			label$36f55b1_3_131:
			r1 = true;
			if( !r1 ) goto label$36f55b1_3_173;
			r8 = r0->pos;
			r10 = r8;
			++r8;
			r0->pos = r8;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r7 = r6->bytes;
			r11 = 1;
			r10 = r10 << r11;
			r10 = *(unsigned short*)(r7 + r10);
			switch(r10) {
				default:
				case 0:
				case 1:
				case 2:
				case 3:
				case 4:
				case 5:
				case 6:
				case 7:
				case 8:
				case 11:
				case 12:
				case 14:
				case 15:
				case 16:
				case 17:
				case 18:
				case 19:
				case 20:
				case 21:
				case 22:
				case 23:
				case 24:
				case 25:
				case 26:
				case 27:
				case 28:
				case 29:
				case 30:
				case 31:
				case 33:
				case 34:
				case 35:
				case 36:
				case 37:
				case 38:
				case 39:
				case 40:
				case 41:
				case 42:
				case 43:
				case 45:
				case 46:
				case 47:
				case 48:
				case 49:
				case 50:
				case 51:
				case 52:
				case 53:
				case 54:
				case 55:
				case 56:
				case 57:
				case 58:
				case 59:
				case 60:
				case 61:
				case 62:
				case 63:
				case 64:
				case 65:
				case 66:
				case 67:
				case 68:
				case 69:
				case 70:
				case 71:
				case 72:
				case 73:
				case 74:
				case 75:
				case 76:
				case 77:
				case 78:
				case 79:
				case 80:
				case 81:
				case 82:
				case 83:
				case 84:
				case 85:
				case 86:
				case 87:
				case 88:
				case 89:
				case 90:
				case 91:
				case 92:
					r1 = r30 ? r30->v.b : 0;
					if( !r1 ) goto label$36f55b1_3_148;
					haxe_format_JsonParser_invalidChar(r0);
					label$36f55b1_3_148:
					r11 = r0->pos;
					--r11;
					r0->pos = r11;
					if( r25 == NULL ) hl_null_access();
					r9 = haxe_format_JsonParser_parseRec(r0);
					r11 = hl_types_ArrayDyn_push(r25,r9);
					r1 = true;
					r31 = hl_alloc_dynbool(r1);
					r30 = r31;
					goto label$36f55b1_3_172;
				case 9:
				case 10:
				case 13:
				case 32:
					goto label$36f55b1_3_172;
				case 44:
					r1 = r30 ? r30->v.b : 0;
					if( !r1 ) goto label$36f55b1_3_165;
					r1 = false;
					r31 = hl_alloc_dynbool(r1);
					r30 = r31;
					goto label$36f55b1_3_166;
					label$36f55b1_3_165:
					haxe_format_JsonParser_invalidChar(r0);
					label$36f55b1_3_166:
					goto label$36f55b1_3_172;
				case 93:
					r1 = false;
					r32 = hl_alloc_dynbool(r1);
					if( r30 != r32 && (!r30 || !r32 || (r30->v.b != r32->v.b)) ) goto label$36f55b1_3_171;
					haxe_format_JsonParser_invalidChar(r0);
					label$36f55b1_3_171:
					return ((vdynamic*)r25);
			}
			label$36f55b1_3_172:
			goto label$36f55b1_3_131;
			label$36f55b1_3_173:
			goto label$36f55b1_3_406;
		case 102:
			r8 = r0->pos;
			r10 = r0->pos;
			r11 = r10;
			++r10;
			r0->pos = r10;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r7 = r6->bytes;
			r12 = 1;
			r11 = r11 << r12;
			r11 = *(unsigned short*)(r7 + r11);
			r12 = 97;
			if( r11 != r12 ) goto label$36f55b1_3_203;
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r19 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r19 + r12);
			r20 = 108;
			if( r12 != r20 ) goto label$36f55b1_3_201;
			r13 = false;
			goto label$36f55b1_3_202;
			label$36f55b1_3_201:
			r13 = true;
			label$36f55b1_3_202:
			goto label$36f55b1_3_204;
			label$36f55b1_3_203:
			r13 = true;
			label$36f55b1_3_204:
			if( r13 ) goto label$36f55b1_3_221;
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r19 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r19 + r12);
			r20 = 115;
			if( r12 != r20 ) goto label$36f55b1_3_219;
			r14 = false;
			goto label$36f55b1_3_220;
			label$36f55b1_3_219:
			r14 = true;
			label$36f55b1_3_220:
			goto label$36f55b1_3_222;
			label$36f55b1_3_221:
			r14 = true;
			label$36f55b1_3_222:
			if( r14 ) goto label$36f55b1_3_239;
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r19 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r19 + r12);
			r20 = 101;
			if( r12 != r20 ) goto label$36f55b1_3_237;
			r15 = false;
			goto label$36f55b1_3_238;
			label$36f55b1_3_237:
			r15 = true;
			label$36f55b1_3_238:
			goto label$36f55b1_3_240;
			label$36f55b1_3_239:
			r15 = true;
			label$36f55b1_3_240:
			if( !r15 ) goto label$36f55b1_3_243;
			r0->pos = r8;
			haxe_format_JsonParser_invalidChar(r0);
			label$36f55b1_3_243:
			r15 = false;
			r9 = hl_alloc_dynbool(r15);
			return r9;
		case 110:
			r8 = r0->pos;
			r10 = r0->pos;
			r11 = r10;
			++r10;
			r0->pos = r10;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r7 = r6->bytes;
			r12 = 1;
			r11 = r11 << r12;
			r11 = *(unsigned short*)(r7 + r11);
			r12 = 117;
			if( r11 != r12 ) goto label$36f55b1_3_275;
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r19 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r19 + r12);
			r20 = 108;
			if( r12 != r20 ) goto label$36f55b1_3_273;
			r13 = false;
			goto label$36f55b1_3_274;
			label$36f55b1_3_273:
			r13 = true;
			label$36f55b1_3_274:
			goto label$36f55b1_3_276;
			label$36f55b1_3_275:
			r13 = true;
			label$36f55b1_3_276:
			if( r13 ) goto label$36f55b1_3_293;
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r19 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r19 + r12);
			r20 = 108;
			if( r12 != r20 ) goto label$36f55b1_3_291;
			r14 = false;
			goto label$36f55b1_3_292;
			label$36f55b1_3_291:
			r14 = true;
			label$36f55b1_3_292:
			goto label$36f55b1_3_294;
			label$36f55b1_3_293:
			r14 = true;
			label$36f55b1_3_294:
			if( !r14 ) goto label$36f55b1_3_297;
			r0->pos = r8;
			haxe_format_JsonParser_invalidChar(r0);
			label$36f55b1_3_297:
			r9 = NULL;
			return r9;
		case 116:
			r8 = r0->pos;
			r10 = r0->pos;
			r11 = r10;
			++r10;
			r0->pos = r10;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r7 = r6->bytes;
			r12 = 1;
			r11 = r11 << r12;
			r11 = *(unsigned short*)(r7 + r11);
			r12 = 114;
			if( r11 != r12 ) goto label$36f55b1_3_328;
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r19 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r19 + r12);
			r20 = 117;
			if( r12 != r20 ) goto label$36f55b1_3_326;
			r13 = false;
			goto label$36f55b1_3_327;
			label$36f55b1_3_326:
			r13 = true;
			label$36f55b1_3_327:
			goto label$36f55b1_3_329;
			label$36f55b1_3_328:
			r13 = true;
			label$36f55b1_3_329:
			if( r13 ) goto label$36f55b1_3_346;
			r11 = r0->pos;
			r12 = r11;
			++r11;
			r0->pos = r11;
			r6 = r0->str;
			if( r6 == NULL ) hl_null_access();
			r19 = r6->bytes;
			r20 = 1;
			r12 = r12 << r20;
			r12 = *(unsigned short*)(r19 + r12);
			r20 = 101;
			if( r12 != r20 ) goto label$36f55b1_3_344;
			r14 = false;
			goto label$36f55b1_3_345;
			label$36f55b1_3_344:
			r14 = true;
			label$36f55b1_3_345:
			goto label$36f55b1_3_347;
			label$36f55b1_3_346:
			r14 = true;
			label$36f55b1_3_347:
			if( !r14 ) goto label$36f55b1_3_350;
			r0->pos = r8;
			haxe_format_JsonParser_invalidChar(r0);
			label$36f55b1_3_350:
			r14 = true;
			r9 = hl_alloc_dynbool(r14);
			return r9;
		case 123:
			r33 = hl_alloc_dynobj();
			r9 = ((vdynamic*)r33);
			r6 = NULL;
			r30 = NULL;
			label$36f55b1_3_357:
			r1 = true;
			if( !r1 ) goto label$36f55b1_3_406;
			r8 = r0->pos;
			r10 = r8;
			++r8;
			r0->pos = r8;
			r34 = r0->str;
			if( r34 == NULL ) hl_null_access();
			r7 = r34->bytes;
			r11 = 1;
			r10 = r10 << r11;
			r10 = *(unsigned short*)(r7 + r10);
			switch(r10) {
				default:
				case 0:
				case 1:
				case 2:
				case 3:
				case 4:
				case 5:
				case 6:
				case 7:
				case 8:
				case 11:
				case 12:
				case 14:
				case 15:
				case 16:
				case 17:
				case 18:
				case 19:
				case 20:
				case 21:
				case 22:
				case 23:
				case 24:
				case 25:
				case 26:
				case 27:
				case 28:
				case 29:
				case 30:
				case 31:
				case 33:
				case 35:
				case 36:
				case 37:
				case 38:
				case 39:
				case 40:
				case 41:
				case 42:
				case 43:
				case 45:
				case 46:
				case 47:
				case 48:
				case 49:
				case 50:
				case 51:
				case 52:
				case 53:
				case 54:
				case 55:
				case 56:
				case 57:
				case 59:
				case 60:
				case 61:
				case 62:
				case 63:
				case 64:
				case 65:
				case 66:
				case 67:
				case 68:
				case 69:
				case 70:
				case 71:
				case 72:
				case 73:
				case 74:
				case 75:
				case 76:
				case 77:
				case 78:
				case 79:
				case 80:
				case 81:
				case 82:
				case 83:
				case 84:
				case 85:
				case 86:
				case 87:
				case 88:
				case 89:
				case 90:
				case 91:
				case 92:
				case 93:
				case 94:
				case 95:
				case 96:
				case 97:
				case 98:
				case 99:
				case 100:
				case 101:
				case 102:
				case 103:
				case 104:
				case 105:
				case 106:
				case 107:
				case 108:
				case 109:
				case 110:
				case 111:
				case 112:
				case 113:
				case 114:
				case 115:
				case 116:
				case 117:
				case 118:
				case 119:
				case 120:
				case 121:
				case 122:
				case 123:
				case 124:
					haxe_format_JsonParser_invalidChar(r0);
					goto label$36f55b1_3_405;
				case 9:
				case 10:
				case 13:
				case 32:
					goto label$36f55b1_3_405;
				case 34:
					if( r6 ) goto label$36f55b1_3_377;
					r1 = r30 ? r30->v.b : 0;
					if( !r1 ) goto label$36f55b1_3_378;
					label$36f55b1_3_377:
					haxe_format_JsonParser_invalidChar(r0);
					label$36f55b1_3_378:
					r34 = haxe_format_JsonParser_parseString(r0);
					r6 = r34;
					goto label$36f55b1_3_405;
				case 44:
					r1 = r30 ? r30->v.b : 0;
					if( !r1 ) goto label$36f55b1_3_387;
					r1 = false;
					r31 = hl_alloc_dynbool(r1);
					r30 = r31;
					goto label$36f55b1_3_388;
					label$36f55b1_3_387:
					haxe_format_JsonParser_invalidChar(r0);
					label$36f55b1_3_388:
					goto label$36f55b1_3_405;
				case 58:
					if( r6 ) goto label$36f55b1_3_391;
					haxe_format_JsonParser_invalidChar(r0);
					label$36f55b1_3_391:
					r35 = haxe_format_JsonParser_parseRec(r0);
					Reflect_setField(r9,r6,r35);
					r34 = NULL;
					r6 = r34;
					r1 = true;
					r31 = hl_alloc_dynbool(r1);
					r30 = r31;
					goto label$36f55b1_3_405;
				case 125:
					if( r6 ) goto label$36f55b1_3_403;
					r1 = false;
					r32 = hl_alloc_dynbool(r1);
					if( r30 != r32 && (!r30 || !r32 || (r30->v.b != r32->v.b)) ) goto label$36f55b1_3_404;
					label$36f55b1_3_403:
					haxe_format_JsonParser_invalidChar(r0);
					label$36f55b1_3_404:
					return r9;
			}
			label$36f55b1_3_405:
			goto label$36f55b1_3_357;
	}
	label$36f55b1_3_406:
	goto label$36f55b1_3_0;
	label$36f55b1_3_407:
	r9 = NULL;
	return r9;
}

String haxe_format_JsonParser_parseString(haxe__format__JsonParser r0) {
	String r9, r17;
	bool r5;
	StringBuf r3, r13;
	int *r18;
	vdynamic *r15;
	vbyte *r8, *r10, *r16;
	int r1, r4, r6, r7, r11, r12, r14, r19, r20;
	r1 = r0->pos;
	r3 = NULL;
	r4 = -1;
	label$36f55b1_4_3:
	r5 = true;
	if( !r5 ) goto label$36f55b1_4_136;
	r6 = r0->pos;
	r7 = r6;
	++r6;
	r0->pos = r6;
	r9 = r0->str;
	if( r9 == NULL ) hl_null_access();
	r8 = r9->bytes;
	r11 = 1;
	r7 = r7 << r11;
	r7 = *(unsigned short*)(r8 + r7);
	r12 = 34;
	if( r7 != r12 ) goto label$36f55b1_4_19;
	goto label$36f55b1_4_136;
	label$36f55b1_4_19:
	r12 = 92;
	if( r7 != r12 ) goto label$36f55b1_4_131;
	if( r3 ) goto label$36f55b1_4_25;
	r13 = (StringBuf)hl_alloc_obj(&t$StringBuf);
	StringBuf_new(r13);
	r3 = r13;
	label$36f55b1_4_25:
	if( r3 == NULL ) hl_null_access();
	r9 = r0->str;
	r12 = r0->pos;
	r12 = r12 - r1;
	r14 = 1;
	r12 = r12 - r14;
	r15 = hl_alloc_dynamic(&t$_i32);
	r15->v.i = r12;
	StringBuf_addSub(r3,r9,r1,r15);
	r11 = r0->pos;
	r12 = r11;
	++r11;
	r0->pos = r11;
	r9 = r0->str;
	if( r9 == NULL ) hl_null_access();
	r10 = r9->bytes;
	r14 = 1;
	r12 = r12 << r14;
	r12 = *(unsigned short*)(r10 + r12);
	r7 = r12;
	r14 = 117;
	if( r12 == r14 ) goto label$36f55b1_4_52;
	r14 = -1;
	if( r4 == r14 ) goto label$36f55b1_4_52;
	r12 = 65533;
	StringBuf_addChar(r3,r12);
	r12 = -1;
	r4 = r12;
	label$36f55b1_4_52:
	switch(r7) {
		default:
		case 0:
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 8:
		case 9:
		case 10:
		case 11:
		case 12:
		case 13:
		case 14:
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
		case 20:
		case 21:
		case 22:
		case 23:
		case 24:
		case 25:
		case 26:
		case 27:
		case 28:
		case 29:
		case 30:
		case 31:
		case 32:
		case 33:
		case 35:
		case 36:
		case 37:
		case 38:
		case 39:
		case 40:
		case 41:
		case 42:
		case 43:
		case 44:
		case 45:
		case 46:
		case 48:
		case 49:
		case 50:
		case 51:
		case 52:
		case 53:
		case 54:
		case 55:
		case 56:
		case 57:
		case 58:
		case 59:
		case 60:
		case 61:
		case 62:
		case 63:
		case 64:
		case 65:
		case 66:
		case 67:
		case 68:
		case 69:
		case 70:
		case 71:
		case 72:
		case 73:
		case 74:
		case 75:
		case 76:
		case 77:
		case 78:
		case 79:
		case 80:
		case 81:
		case 82:
		case 83:
		case 84:
		case 85:
		case 86:
		case 87:
		case 88:
		case 89:
		case 90:
		case 91:
		case 93:
		case 94:
		case 95:
		case 96:
		case 97:
		case 99:
		case 100:
		case 101:
		case 103:
		case 104:
		case 105:
		case 106:
		case 107:
		case 108:
		case 109:
		case 111:
		case 112:
		case 113:
		case 115:
			r9 = (String)s$Invalid_escape_sequence_;
			r17 = String_fromCharCode(r7);
			r9 = String___add__(r9,r17);
			r17 = (String)s$_at_position_;
			r9 = String___add__(r9,r17);
			r12 = r0->pos;
			r14 = 1;
			r12 = r12 - r14;
			r18 = &r12;
			r16 = hl_itos(r12,r18);
			r17 = String___alloc__(r16,r12);
			r9 = String___add__(r9,r17);
			hl_throw((vdynamic*)r9);
		case 34:
		case 47:
		case 92:
			StringBuf_addChar(r3,r7);
			goto label$36f55b1_4_128;
		case 98:
			r12 = 8;
			StringBuf_addChar(r3,r12);
			goto label$36f55b1_4_128;
		case 102:
			r12 = 12;
			StringBuf_addChar(r3,r12);
			goto label$36f55b1_4_128;
		case 110:
			r12 = 10;
			StringBuf_addChar(r3,r12);
			goto label$36f55b1_4_128;
		case 114:
			r12 = 13;
			StringBuf_addChar(r3,r12);
			goto label$36f55b1_4_128;
		case 116:
			r12 = 9;
			StringBuf_addChar(r3,r12);
			goto label$36f55b1_4_128;
		case 117:
			r9 = (String)s$0x;
			r17 = r0->str;
			if( r17 == NULL ) hl_null_access();
			r12 = r0->pos;
			r14 = 4;
			r15 = hl_alloc_dynamic(&t$_i32);
			r15->v.i = r14;
			r17 = String_substr(r17,r12,r15);
			r9 = String___add__(r9,r17);
			r15 = Std_parseInt(r9);
			r12 = r15 ? r15->v.i : 0;
			r14 = r0->pos;
			r19 = 4;
			r14 = r14 + r19;
			r0->pos = r14;
			r19 = -1;
			if( r4 == r19 ) goto label$36f55b1_4_121;
			r19 = 56320;
			if( r12 < r19 ) goto label$36f55b1_4_103;
			r19 = 57343;
			if( r19 >= r12 ) goto label$36f55b1_4_108;
			label$36f55b1_4_103:
			r14 = 65533;
			StringBuf_addChar(r3,r14);
			r14 = -1;
			r4 = r14;
			goto label$36f55b1_4_120;
			label$36f55b1_4_108:
			r19 = 55296;
			r14 = r4 - r19;
			r19 = 10;
			r14 = r14 << r19;
			r20 = 56320;
			r19 = r12 - r20;
			r14 = r14 + r19;
			r19 = 65536;
			r14 = r14 + r19;
			StringBuf_addChar(r3,r14);
			r14 = -1;
			r4 = r14;
			label$36f55b1_4_120:
			goto label$36f55b1_4_128;
			label$36f55b1_4_121:
			r19 = 55296;
			if( r12 < r19 ) goto label$36f55b1_4_127;
			r19 = 56319;
			if( r19 < r12 ) goto label$36f55b1_4_127;
			r4 = r12;
			goto label$36f55b1_4_128;
			label$36f55b1_4_127:
			StringBuf_addChar(r3,r12);
	}
	label$36f55b1_4_128:
	r12 = r0->pos;
	r1 = r12;
	goto label$36f55b1_4_135;
	label$36f55b1_4_131:
	r12 = 0;
	if( r7 != r12 ) goto label$36f55b1_4_135;
	r9 = (String)s$Unclosed_string;
	hl_throw((vdynamic*)r9);
	label$36f55b1_4_135:
	goto label$36f55b1_4_3;
	label$36f55b1_4_136:
	r7 = -1;
	if( r4 == r7 ) goto label$36f55b1_4_142;
	if( r3 == NULL ) hl_null_access();
	r6 = 65533;
	StringBuf_addChar(r3,r6);
	r6 = -1;
	label$36f55b1_4_142:
	if( r3 ) goto label$36f55b1_4_152;
	r9 = r0->str;
	if( r9 == NULL ) hl_null_access();
	r7 = r0->pos;
	r7 = r7 - r1;
	r11 = 1;
	r7 = r7 - r11;
	r15 = hl_alloc_dynamic(&t$_i32);
	r15->v.i = r7;
	r9 = String_substr(r9,r1,r15);
	return r9;
	label$36f55b1_4_152:
	if( r3 == NULL ) hl_null_access();
	r9 = r0->str;
	r7 = r0->pos;
	r7 = r7 - r1;
	r11 = 1;
	r7 = r7 - r11;
	r15 = hl_alloc_dynamic(&t$_i32);
	r15->v.i = r7;
	StringBuf_addSub(r3,r9,r1,r15);
	r9 = StringBuf_toString(r3);
	return r9;
}

void haxe_format_JsonParser_invalidChar(haxe__format__JsonParser r0) {
	String r4, r8;
	int *r7;
	vbyte *r3, *r5;
	int r1, r2, r6;
	r1 = r0->pos;
	--r1;
	r0->pos = r1;
	r1 = r0->pos;
	r4 = r0->str;
	if( r4 == NULL ) hl_null_access();
	r3 = r4->bytes;
	r4 = (String)s$Invalid_char_;
	r6 = 1;
	r2 = r1 << r6;
	r2 = *(unsigned short*)(r3 + r2);
	r7 = &r2;
	r5 = hl_itos(r2,r7);
	r8 = String___alloc__(r5,r2);
	r4 = String___add__(r4,r8);
	r8 = (String)s$_at_position_;
	r4 = String___add__(r4,r8);
	r2 = r0->pos;
	r7 = &r2;
	r5 = hl_itos(r2,r7);
	r8 = String___alloc__(r5,r2);
	r4 = String___add__(r4,r8);
	hl_throw((vdynamic*)r4);
}

void haxe_format_JsonParser_invalidNumber(haxe__format__JsonParser r0,int r1) {
	String r2, r6;
	vdynamic *r8;
	int *r4;
	vbyte *r5;
	int r3, r7;
	r2 = (String)s$Invalid_number_at_position_;
	r3 = r1;
	r4 = &r3;
	r5 = hl_itos(r3,r4);
	r6 = String___alloc__(r5,r3);
	r2 = String___add__(r2,r6);
	r6 = (String)s$e265492;
	r2 = String___add__(r2,r6);
	r6 = r0->str;
	if( r6 == NULL ) hl_null_access();
	r7 = r0->pos;
	r7 = r7 - r1;
	r8 = hl_alloc_dynamic(&t$_i32);
	r8->v.i = r7;
	r6 = String_substr(r6,r1,r8);
	r2 = String___add__(r2,r6);
	hl_throw((vdynamic*)r2);
}

